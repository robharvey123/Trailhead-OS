...
export async function GET(
  ...
  const [contact, account, workstreams, companySettings, bankAccount, payments, remittanceAccounts] = await Promise.all([
    ...
    getRemittanceAccountsForCurrency(invoiceCurrency, supabase).catch(() => []),
  ])
  const workstream =
    workstreams.find((item) => item.id === invoice.workstream_id) ?? null
  const buffer = await renderInvoicePdf(invoice, contact, workstream, companySettings, bankAccount, account, payments, remittanceAccounts)
