...
export function formatTimeEntry(row: TimeEntryRow) {
  ...
  return {
    ...
    description: row.description,
    client_description: (row as { client_description?: string | null }).client_description ?? null,
    billable: row.billable,
    rate_snapshot: rate,
    currency: row.currency_snapshot,
    amount: Math.round(hours * rate * 100) / 100,
    billed: Boolean((row as { billed?: boolean }).billed),
    invoice_id: (row as { invoice_id?: string | null }).invoice_id ?? null,
    source: row.source,
    engagement: engagement ? { id: engagement.id, name: engagement.name, code: engagement.code } : null,
    project: project ? { id: project.id, name: project.name } : null,
    account: account ? { id: account.id, name: account.name } : null,
