...
export default function EndProjectDialog({
  ...
  // `thmock` so the design tokens resolve in this body-portal-free overlay.
  return (
    <div className="thmock fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(15,23,42,0.45)', padding: 16 }} onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="panel" style={{ width: '100%', maxWidth: 440, padding: 20 }}>
        ...
        <div style={{ display: 'grid', gap: 14, marginTop: 8 }}>
          ...
          <div>
            ...
            <textarea className="w-full rounded-[5px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-sm text-[var(--text)] outline-none focus:border-[var(--accent)] min-h-[3rem] resize-y" value={reason} onChange={(e) => setReason(e.target.value)} />
          </div>
