...
type ContactDup = { a: { id: string; name: string }; b: { id: string; name: string }; reason: 'name' | 'email' }
...
type Tab = 'contacts' | 'accounts' | 'threads'
...
export default function LinkBacklogClient() {
  ...
  return (
    <div className="thmock space-y-4">
      ...
      {tab === 'accounts' ? (
        <div className="os-card p-4">
          <p className="mb-3 text-sm text-[color:var(--text-2)]">Near-duplicate account names. Merge tooling arrives in the next step — for now this is a review list.</p>
          {data.account_duplicates.length === 0 ? <div className="empty">No duplicate accounts detected.</div> : (
            <table className="data-table">
              <thead><tr><th>Account A</th><th>Account B</th><th style={{ textAlign: 'right' }}>Similarity</th></tr></thead>
              <tbody>
