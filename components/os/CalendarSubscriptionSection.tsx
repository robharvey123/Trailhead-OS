...
export default function CalendarSubscriptionSection({ appUrl, icalSecret, workstreams }: Props) {
  ...
  return (
    <section className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-6">
      <p className="text-xs uppercase tracking-[0.24em] text-white0">Calendar</p>
      ...
      <div className="mt-5 space-y-4">
        <div>
          <label className="text-xs uppercase tracking-[0.2em] text-white0">Subscription URL</label>
          <input
            readOnly
            value={maskedUrl}
            className="mt-2 w-full rounded-2xl border border-[#2A2A3A] bg-[#13131E] px-4 py-3 text-sm text-[#9CA3AF] outline-none"
          />
        </div>
        ...
        <details className="rounded-2xl border border-[#2A2A3A] bg-[#13131E] px-4 py-3 text-sm text-[#9CA3AF]">
          <summary className="cursor-pointer font-medium text-white">How to subscribe</summary>
          <div className="mt-4 space-y-4 text-[#9CA3AF]">
            <div>
              <p className="font-medium text-[#9CA3AF]">iPhone/iPad</p>
            ...
            <div>
              <p className="font-medium text-[#9CA3AF]">Mac</p>
              <p>1. Click &quot;Open in Apple Calendar&quot; above</p>
              <p>2. Click Subscribe in the dialog that opens</p>
              <p>3. Click OK</p>
            </div>
            <div>
              <p className="font-medium text-[#9CA3AF]">Google Calendar</p>
            ...
            <div>
              <p className="font-medium text-[#9CA3AF]">Outlook</p>
              <p>1. Copy the URL</p>
        ...
        <div className="rounded-2xl border border-[#2A2A3A] bg-[#13131E] p-4">
          <p className="text-sm font-medium text-white">Or subscribe to a single workstream:</p>
          <div className="mt-4 space-y-3">
            {workstreams.slice(0, 5).map((workstream) => {
              ...
              return (
                <div
                  key={workstream.id}
                  className="rounded-2xl border border-[#2A2A3A] bg-[#1A1A28]/60 p-3"
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-white">{workstream.label}</p>
                      <p className="mt-1 break-all text-xs text-[#9CA3AF]">{maskedWorkstreamUrl}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => copyUrl(workstreamUrl, copyKey)}
                      className="rounded-2xl border border-[#2A2A3A] px-3 py-2 text-xs text-white transition hover:border-[#B8FF00]/40"
                    >
