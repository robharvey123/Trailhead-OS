  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.32em] text-white0">Command centre</p>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-[1.1fr_1fr_0.95fr]">
        <section className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-[#9CA3AF]">Due today plus anything pinned to the master list.</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-5">
            {todaysTasks.length === 0 ? (
              <div className="rounded-[1.75rem] border border-dashed border-[#2A2A3A] px-4 py-10 text-center">
                <p className="text-base font-medium text-[#9CA3AF]">No tasks in focus yet.</p>
                <p className="mt-2 text-sm text-white0">Add the first one above to get the board moving.</p>
              </div>
            ) : (
              Object.entries(groupedToday).map(([group, tasks]) => (
                <div key={group}>
                  <h3 className="mb-3 text-xs uppercase tracking-[0.24em] text-white0">{group}</h3>
                  <div className="space-y-3">
                    {tasks.map((task) => (
                      <Link
                        key={task.id}
                        href={getTaskHref(task)}
                        className="block rounded-3xl border border-[#2A2A3A] bg-[#13131E] p-4 transition hover:border-[#2A2A3A] hover:bg-[#0C0C14]"
                      >
        ...
        <section className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-5">
          <div>
            <h2 className="text-lg font-semibold text-white">Board activity</h2>
            <p className="text-sm text-[#9CA3AF]">A quick read on what each workstream needs next.</p>
          </div>
            ...
            {workstreamSummaries.map((summary) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-[1.75rem] border p-5 transition hover:-translate-y-0.5 hover:border-[#2A2A3A] ${classes.card}`}
                >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {summary.column_counts.map((column) => (
                      <div key={column.column_id} className="rounded-2xl border border-[#2A2A3A]/70 bg-[#13131E] px-3 py-3">
                        <p className="text-xs uppercase tracking-[0.2em] text-white0">{column.label}</p>
                        <p className="mt-2 text-lg font-semibold text-white">{column.task_count}</p>
                      </div>
                    ))}
                  </div>
                </Link>
        ...
        <section className="space-y-6">
          <div className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-5">
            <div>
              <h2 className="text-lg font-semibold text-white">Upcoming</h2>
              <p className="text-sm text-[#9CA3AF]">What&apos;s landing over the next week.</p>
            </div>

            <div className="mt-5 space-y-4">
              {upcomingItems.length === 0 ? (
                <p className="rounded-3xl border border-dashed border-[#2A2A3A] px-4 py-6 text-sm text-white0">
                  Nothing due in the next seven days.
                </p>
              ) : (
                Object.entries(groupedUpcoming).map(([date, items]) => (
                  <div key={date}>
                    <h3 className="mb-2 text-xs uppercase tracking-[0.24em] text-white0">
                      {formatTaskDate(date)}
                    </h3>
          ...
          <div className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-5">
            <div>
              ...
              <p className="text-sm text-[#9CA3AF]">The last edits across all workstreams.</p>
            </div>
              ...
              {recentNotes.length === 0 ? (
                ...
                recentNotes.map((note) => (
                  ...
                  >
                    ...
                    <p className="mt-3 whitespace-pre-wrap text-sm text-[#9CA3AF]">{note.body}</p>
                    <p className="mt-3 text-xs text-white0">{formatDateTime(note.updated_at)}</p>
                  </Link>
                ))
              )}
            </div>
