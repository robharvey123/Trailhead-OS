  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="os-eyebrow">Command centre</p>
          <h1 className="os-page-title mt-2">Dashboard</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-[1.1fr_1fr_0.95fr]">
        <section className="os-card p-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="os-section-title">Today&apos;s tasks</h2>
              <p className="text-sm text-[color:var(--text-2)]">Due today plus anything pinned to the master list.</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-5">
            {todaysTasks.length === 0 ? (
              <div className="rounded-[1.75rem] border border-dashed border-[color:var(--border)] px-4 py-10 text-center">
                <p className="text-base font-medium text-[color:var(--text-2)]">No tasks in focus yet.</p>
                <p className="mt-2 text-sm text-[color:var(--text-3)]">Add the first one above to get the board moving.</p>
              </div>
            ) : (
              Object.entries(groupedToday).map(([group, tasks]) => (
                <div key={group}>
                  <h3 className="mb-3 text-xs uppercase tracking-[0.24em] text-[color:var(--text-3)]">{group}</h3>
                  <div className="space-y-3">
                    {tasks.map((task) => (
                      <Link
                        key={task.id}
                        href={getTaskHref(task)}
                        className="block rounded-3xl border border-[color:var(--border)] bg-white p-4 transition hover:border-[color:var(--border-light)] hover:bg-[var(--surface-2)]"
                      >
        ...
        <section className="os-card p-5">
          <div>
            <h2 className="os-section-title">Board activity</h2>
            <p className="text-sm text-[color:var(--text-2)]">A quick read on what each workstream needs next.</p>
          </div>
            ...
            {workstreamSummaries.map((summary) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-[1.75rem] border p-5 transition hover:-translate-y-0.5 hover:border-[color:var(--border-light)] ${classes.card}`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3">
                        <span className={`h-3 w-3 rounded-full ${classes.dot}`} />
                        <h3 className="text-base font-semibold text-[color:var(--text)]">{summary.label}</h3>
                      </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {summary.column_counts.map((column) => (
                      <div key={column.column_id} className="rounded-2xl border border-[color:var(--border)] bg-[var(--surface-2)] px-3 py-3">
                        <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--text-3)]">{column.label}</p>
                        <p className="mt-2 text-lg font-semibold text-[color:var(--text)]">{column.task_count}</p>
                      </div>
                    ))}
                  </div>
                </Link>
        ...
        <section className="space-y-6">
          <div className="os-card p-5">
            <div>
              <h2 className="os-section-title">Upcoming</h2>
              <p className="text-sm text-[color:var(--text-2)]">What&apos;s landing over the next week.</p>
            </div>

            <div className="mt-5 space-y-4">
              {upcomingItems.length === 0 ? (
                <p className="rounded-3xl border border-dashed border-[color:var(--border)] px-4 py-6 text-sm text-[color:var(--text-3)]">
                  Nothing due in the next seven days.
                </p>
              ) : (
                Object.entries(groupedUpcoming).map(([date, items]) => (
                  <div key={date}>
                    <h3 className="mb-2 text-xs uppercase tracking-[0.24em] text-[color:var(--text-3)]">
                      {formatTaskDate(date)}
                    </h3>
                      ...
                      {items.map((item) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-[color:var(--text-2)]">
          ...
          <div className="os-card p-5">
            <div>
              <h2 className="os-section-title">Recent notes</h2>
              <p className="text-sm text-[color:var(--text-2)]">The last edits across all workstreams.</p>
            </div>
              ...
              {recentNotes.length === 0 ? (
                ...
                recentNotes.map((note) => (
                  ...
                  >
                    ...
                    <p className="mt-3 whitespace-pre-wrap text-sm text-[color:var(--text-2)]">{note.body}</p>
