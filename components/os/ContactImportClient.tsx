  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.32em] text-white/60">Clients</p>
          <h1 className="mt-2 text-3xl font-semibold text-white">Import contacts</h1>
      ...
      {step === 'upload' && (
        <div
          ...
          onDrop={handleDrop}
          className="flex flex-col items-center gap-4 rounded-[2rem] border-2 border-dashed border-[#2A2A3A] bg-[#1A1A28] px-8 py-16 text-center transition hover:border-[#B8FF00]/40"
        >
          <div className="rounded-full bg-[#B8FF00]/10 p-4">
            <svg className="h-8 w-8 text-[#B8FF00]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
            </svg>
          </div>
          <div>
            <p className="text-lg font-medium text-white">Drag &amp; drop a CSV file here</p>
            <p className="mt-1 text-sm text-[#9CA3AF]">or click to browse</p>
          </div>
          <input
            ref={fileInputRef}
            type="file"
          ...
          <button
            ...
            className="rounded-2xl bg-white px-6 py-3 text-sm font-semibold text-[#0C0C14] transition hover:bg-[#B8FF00]/90"
          >
            Choose file
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button
                onClick={handleReset}
                className="rounded-xl border border-[#2A2A3A] px-3 py-2 text-xs text-[#9CA3AF] transition hover:border-white/40 hover:text-white"
              >
            ...
            <div className="space-y-3">
              {csvHeaders.map((header) => (
                ...
                >
                  <span className="truncate text-sm text-white">{header}</span>
                  <span className="hidden text-[#9CA3AF] md:block">→</span>
                  <select
                    ...
                    className="rounded-xl border border-[#2A2A3A] bg-[#0C0C14] px-3 py-2 text-sm text-white"
                  >
          ...
          <div className="flex justify-end gap-3">
            <button
              onClick={handleReset}
              className="rounded-2xl border border-[#2A2A3A] px-5 py-3 text-sm font-medium text-white transition hover:border-white/40"
            >
              Cancel
            </button>
            <button
              ...
              className="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-[#0C0C14] transition hover:bg-[#B8FF00]/90 disabled:opacity-40"
            >
              Preview import
            </button>
          </div>
        ...
        <div className="space-y-6">
                ...
                <tbody>
                  {mappedRows.slice(0, 20).map((row, i) => {
                    ...
                    return (
                      <tr
                        key={i}
                        className={`border-t border-[#2A2A3A] ${!hasName ? 'opacity-40' : ''}`}
                      >
                        <td className="py-3 pr-4 text-[#9CA3AF]">{i + 1}</td>
                        {CONTACT_FIELDS.filter((f) =>
                          Object.values(mapping).includes(f.key)
                        ).map((f) => (
                          <td key={f.key} className="py-3 pr-4 text-white">
                            {row[f.key] || <span className="text-[#9CA3AF]">—</span>}
                          </td>
                        ))}
                      </tr>
                    )
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              ...
              className="rounded-2xl bg-[#B8FF00] px-5 py-3 text-sm font-semibold text-[#0C0C14] transition hover:bg-[#B8FF00]/80 disabled:opacity-40"
            >
      ...
      {step === 'result' && result && (
        <div className="space-y-6">
          <div className="rounded-[2rem] border border-[#2A2A3A] bg-[#1A1A28] p-6">
            ...
            {result.rejected.length > 0 && (
              <div className="mt-6">
                <h3 className="mb-2 text-sm font-medium text-white">Rejected rows</h3>
                <div className="space-y-1">
                  {result.rejected.map((r) => (
                    <p key={r.row} className="text-sm text-[#9CA3AF]">
                      Row {r.row}: {r.reason}
                    </p>
                  ))}
                </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Link
              href="/crm/contacts"
              className="rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-[#0C0C14] transition hover:bg-[#B8FF00]/90"
            >
              View contacts
            </Link>
          </div>
